<?php
 ...
 if(isset($_REQUEST['sand_cent'])){
		...
		$regDate =date('Y-m-d'); 
		   ...
		   $insQ=$db->query("INSERT INTO `group_tbl` SET `gid`=1 , `gname`='$gNm',`createddate`='$regDate' ,`status`=1 ") or die("Error");  
	 ?>
	 <script>
   ...
   while(($emapData = fgetcsv($handle,10000, ",")) !== FALSE)
   {
              ...
              $insQ=$db->query("INSERT INTO `group_tbl` SET `gid`='$emapData[0]',`gname`='$emapData[1]',`created`='$regDate',`status`=1 ") or die("Error");  
   }
    ?>
	 <script>
